...
class DepthCompletionFrontNet(nn.Module):
    def __init__(self, args):
        ...
        self.convt2_road = convt_bn_relu(in_channels=(128 + 128 + 64),
                                    out_channels=64,
                                    kernel_size=kernel_size,
                                    stride=stride,
                                    padding=1,
                                    output_padding=1)
        self.convt1_road = convt_bn_relu(in_channels=64 + 64 + 64,
                                    out_channels=64,
                                    kernel_size=kernel_size,
                                    stride=1,
                                    padding=1)
        self.convtf_road = conv_bn_relu(in_channels=64 + 64 + 64,
                                   out_channels=1, # 二分类
                                   kernel_size=1,
                                   stride=1,
                                   bn=False,
    ...
    def forward(self, x):
       ...
       #     print("\n    3x3 TransposeConv_bn_relu {} --> output shape: {}".format(2, convt4.shape))
        ...
        if print_model:
            print("\n    3x3 TransposeConv_bn_relu {} --> output shape: {}".format(4, convt2_.shape))
        y_ = torch.cat((convt2_road, conv2_cat), 1)
        if print_model: 
            print("\n    skip connection from ResNet Block{}".format(1))
        ...
        if print_model:
            print("\n    3x3 TransposeConv_bn_relu {} --> output shape: {}".format(5, convt1_.shape))
        y_ = torch.cat((convt1_road, conv1_cat), 1)
        if print_model:
            print("\n    skip connection from the concat feature of first layer")
