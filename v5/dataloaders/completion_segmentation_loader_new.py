# -*- coding: utf-8 -*-
"""
Created on Mon Mar 30 20:07:51 2020
...
def get_paths_and_transform(split, args):
    ...
    if split == "train":
        ...
        glob_gt = os.path.join(
            ...
            'test_completion_output_train_rename/*.png'
            
        )
        
        # 用于语义分割的label
        glob_segmentation_label = os.path.join(
            args.data_folder,
            #'data_depth_annotated/train/*_sync/proj_depth/groundtruth/image_0[2,3]/*.png'
            'label/*.png'
        )
...
#                            ps[-2:-1] + ['data'] + ps[-1:])
    ...
    # 测试, 输入为点云强度和rgb
    elif split == 'test_completion_segmentation':
        transform = no_transform
        glob_d = os.path.join(
            args.data_folder,
            'reflectance/*.png'
    ...
    if glob_gt is not None:
        ...
        paths_segmentation_label = sorted(glob.glob(glob_segmentation_label))
    else:  
        # test only has d or rgb
        paths_rgb = sorted(glob.glob(glob_rgb))
        paths_gt = [None] * len(paths_rgb)
        
        paths_segmentation_label = [None]*len(paths_rgb)
        paths_d = [None]*len(paths_rgb)
    ...
    paths = {"rgb": paths_rgb, "d": paths_d, "gt": paths_gt, "segmentation_label": paths_segmentation_label}
...
#        assert isinstance(size, float) or (isinstance(size, collections.Iterable) and len(size) == 2)
#        self.size = size
...
#        """
#        Args:
#            img (numpy.ndarray (C x H x W)): Image to be sized.
#        Returns:
#            img (numpy.ndarray (C x H x W)): Resized image.
#        """
#        if img.ndim == 3:
#            return skimage.transform.resize(img, self.size, order=0)
#        elif img.ndim == 2:
#            return skimage.transform.resize(img, self.size, order=0)
#        else:
#            RuntimeError(
...
def train_transform(rgb, sparse, target, segmentation_label, rgb_near, args):
    ...
    return rgb, sparse, target, segmentation_label, rgb_near


def val_transform(rgb, sparse, target, segmentation_label, rgb_near, args):
...
def no_transform(rgb, sparse, target, segmentation_label, rgb_near, args):
    return rgb, sparse, target, segmentation_label, rgb_near
...
class KittiDepth(data.Dataset):
    ...
    def __getraw__(self, index):
        ...
        target = depth_read(self.paths['gt'][index], self.args) if \
            self.paths['gt'][index] is not None else None
            
        segmentation_label = label_read(self.paths['segmentation_label'][index], self.args) if \
            self.paths['segmentation_label'][index] is not None else None
        rgb_near = get_rgb_near(self.paths['rgb'][index], self.args) if \
            self.split == 'train' and self.args.use_pose else None
        return rgb, sparse, target, segmentation_label, rgb_near

    def __getitem__(self, index):
        rgb, sparse, target, segmentation_label, rgb_near = self.__getraw__(index)
        rgb, sparse, target, segmentation_label, rgb_near = self.transform(rgb, sparse, target,segmentation_label,
                                                       rgb_near, self.args)
        ...
        #gray = None
        candidates = {"rgb":rgb, "d":sparse, "gt":target, "segmentation_label":segmentation_label ,\
