# -*- coding: utf-8 -*-
"""
Created on Mon Mar 30 09:12:18 2020
...
def get_paths_and_transform(split, args):
    ...
    if split == "train":
        ...
        # 用于语义分割的label
        glob_completion_gt = os.path.join(
            args.data_folder,
            #'data_depth_annotated/train/*_sync/proj_depth/groundtruth/image_0[2,3]/*.png'
            'label/*.png'
        )
...
#                            ps[-2:-1] + ['data'] + ps[-1:])
    ...
    if glob_gt is not None:
        ...
        paths_completion_gt = sorted(glob.glob(glob_completion_gt))
    else:  
    ...
    paths = {"rgb": paths_rgb, "d": paths_d, "gt": paths_gt, "completion_gt": paths_completion_gt}
...
def rgb_read(filename):
    ...
    img_file = img_file.resize((80, 80))
...
def train_transform(rgb, sparse, target, completion_target, rgb_near, args):
    ...
    completion_target = transform_geometric(completion_target)
    if rgb is not None:
    ...
    return rgb, sparse, target, completion_target, rgb_near


def val_transform(rgb, sparse, target, completion_target, rgb_near, args):
    ...
    if completion_target is not None:
        completion_target = transform(completion_target)
    if rgb_near is not None:
        rgb_near = transform(rgb_near)
    return rgb, sparse, target, completion_target, rgb_near


def no_transform(rgb, sparse, target, completion_target, rgb_near, args):
    return rgb, sparse, target, completion_target, rgb_near
...
class KittiDepth(data.Dataset):
    ...
    def __getraw__(self, index):
        ...
        completion_target = label_read(self.paths['completion_gt'][index]) if \
            self.paths['completion_gt'][index] is not None else None
        rgb_near = get_rgb_near(self.paths['rgb'][index], self.args) if \
            self.split == 'train' and self.args.use_pose else None
        return rgb, sparse, target, completion_target, rgb_near

    def __getitem__(self, index):
        rgb, sparse, target, completion_target, rgb_near = self.__getraw__(index)
        rgb, sparse, target, completion_target, rgb_near = self.transform(rgb, sparse, target,completion_target,
                                                       rgb_near, self.args)
        ...
        candidates = {"rgb":rgb, "d":sparse, "gt":target, "completion_gt":completion_target ,\
