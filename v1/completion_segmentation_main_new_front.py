...
args.result = os.path.join('..', 'results/v1')
...
def acc(prediction, label, num_class=2):
    ...
    for i in range(bs):
        ...
        count = np.bincount(result_, minlength=4)
...
def iterate(mode, args, loader, model, optimizer, logger, best_acc, epoch):
    ...
    for i, batch_data in enumerate(loader):
        ...
        with torch.no_grad():
            # 保存预测结果为图片
            logger.conditional_save_pred(mode, i, lane_pred, epoch)
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch%5==0):
            ...
            print('Validation Time Cost: ', (time.clock()-start)/114)
    ...
    #plot.plot('lane_val', args.epochs, lane_acc_list = val_lane_acc_list)
        ...
        with open('log/train_lane_loss_v1_'+time.strftime('%Y-%m-%d-%H:%M:%S',time.localtime(time.time()))+'.txt', 'w+') as f:
            train_lane_loss_list_w = [str(line) for line in train_lane_loss_list]
            f.writelines('\n'.join(train_lane_loss_list_w))
        with open('log/train_lane_acc_v1_'+time.strftime('%Y-%m-%d-%H:%M:%S',time.localtime(time.time()))+'.txt', 'w+') as f:
            train_lane_acc_list_w = [str(line) for line in train_lane_acc_list]
            f.writelines('\n'.join(train_lane_acc_list_w))
        with open('log/train_total_acc_v1_'+time.strftime('%Y-%m-%d-%H:%M:%S',time.localtime(time.time()))+'.txt', 'w+') as f:
            train_total_acc_list_w = [str(line) for line in train_total_acc_list]
            f.writelines('\n'.join(train_total_acc_list_w))
        with open('log/val_lane_acc_v1_'+time.strftime('%Y-%m-%d-%H:%M:%S',time.localtime(time.time()))+'.txt', 'w+') as f:
            val_lane_acc_list_w = [str(line) for line in val_lane_acc_list]
            f.writelines('\n'.join(val_lane_acc_list_w))
        with open('log/val_total_acc_v1_'+time.strftime('%Y-%m-%d-%H:%M:%S',time.localtime(time.time()))+'.txt', 'w+') as f:
