...
parser.add_argument('-e', '--evaluate', default='/home/linux/Program/lane_segmentation/result/v3/train_sk_add/model_best.pth.tar', type=str, metavar='PATH')
parser.add_argument('--cpu', action="store_true", help='run on cpu')

args = parser.parse_args()
args.use_pose = ("photo" in args.train_mode)
# args.pretrained = not args.no_pretrained
args.result = os.path.join('..', 'result/v3/test_sk')
...
def acc(prediction, label, num_class=2):
    ...
    return acc_total, acc_lane,precision,F1_score
...
def iterate(mode, args, loader, model, optimizer, logger, best_acc, epoch):
    ...
    for i, batch_data in enumerate(loader):
        ...
        if mode == 'train':
            ...
            # 准确率
            #road_acc = acc(road_pred.data.cpu(), road_label.cpu())
            total_acc, lane_acc,precision_1,F1_score_1 = acc(lane_pred.data.cpu(), lane_label.cpu())
            lane_acc_lst.append(lane_acc.item())
            total_acc_lst.append(total_acc.item())
    ...
    if mode=='train':
        ...
        print('lane acc: {}'.format(lane_acc_mean), 'total acc: {}'.format(total_acc_mean),'precision: {}'.format(precision_mean) ,'F1 score: {}'.format(F1_score_mean))
    print('\n-------------------------epoch '+str(epoch)+'-----------------------------\n')
...
#    logger.conditional_summarize(mode, avg, is_best)
    ...
    if mode == 'train':
        ...
        return acc_avg, is_best, lane_acc_mean, total_acc_mean,precision_mean,F1_score_mean
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        start = time.clock()
        result, is_best, val_lane_acc, val_total_acc,val_precision,val_F1_score = iterate("val", args, val_loader, model, None, logger, best_acc, epoch)  # evaluate on validation set
    ...
    #plot.plot('lane_val', args.epochs, lane_acc_list = val_lane_acc_list)
        ...
        with open('log/val_precision_v3_'+time.strftime('%Y-%m-%d-%H:%M:%S',time.localtime(time.time()))+'.txt', 'w+') as f:
            val_precision_list_w = [str(line) for line in val_precision_list]
            f.writelines('\n'.join(val_precision_list_w))
        with open('log/val_F1_score_v3_'+time.strftime('%Y-%m-%d-%H:%M:%S',time.localtime(time.time()))+'.txt', 'w+') as f:
