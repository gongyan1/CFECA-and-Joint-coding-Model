...
class DepthCompletionFrontNet(nn.Module):
    def __init__(self, args):
        ...
        self.conv_fc1 = nn.Conv2d(planes, planes//k_k, 1, bias=False)
        # self.bn_fc1 = nn.BatchNorm2d(planes//2)
        self.conv_fc2 = nn.Conv2d(planes//k_k, planes, 1, bias=False)
        self.D1 = planes//2
    ...
    def forward(self, x):
        ...
        if self.modality == 'rgbd' or self.modality == 'gd':
            ...
            # print(d1.shape)           # torch.Size([1, 128, 352, 1216])
            d = self.avg_pool(d1)
            d = self.conv_fc1(d)
            # d = self.bn_fc1(d)
            d = F.relu(d)
            ...
            # print(d.shape)                # torch.Size([1, 1, 128, 1, 1])
            d = d.view(-1, 2, self.D1, 1, 1)
            # print(d.shape)                # torch.Size([1, 2, 64, 1, 1])
            d = F.softmax(d, 1)
            # print(d.shape)                # torch.Size([1, 2, 64, 1, 1])
            # 0 1 如果交换会不会有效果
            x1_cloud = conv1_d * d[:, 0, :, :, :].squeeze(1)
            # print(x1_cloud.shape)          # torch.Size([1, 64, 352, 1216])
            x1 = conv1_img * d[:, 1, :, :, :].squeeze(1)
            # print(x1.shape)                # torch.Size([1, 64, 352, 1216])
            conv1 = x1_cloud + x1
            if print_model:
