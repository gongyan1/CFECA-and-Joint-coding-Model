...
from completion_segmentation_model_v3_eca_attention_2_add import DepthCompletionFrontNet
...
args.result = os.path.join('..', 'result/v3/train_eca')
...
def iterate(mode, args, loader, model, optimizer, logger, best_acc, epoch):
    ...
    for i, batch_data in enumerate(loader):
        ...
        lane_pred = pred
        start_ = time.process_time() # 不计入时间
        if mode == 'train':
            # 语义分割loss
        ...
        nonsense += (time.process_time()-start_)

    print('total cost time: ', time.process_time()-start_val-nonsense)
    if mode=='train':
