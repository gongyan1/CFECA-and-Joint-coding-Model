...
# from completion_segmentation_model_v3_sk_attention import DepthCompletionFrontNet
# from completion_segmentation_model_v3_sk_attention_2_add import DepthCompletionFrontNet

from completion_segmentation_model_v5_eca_attention_1 import DepthCompletionFrontNet
...
parser.add_argument('-e', '--evaluate', default='/home/first/桌面/gongyan/lane_code/result/v5/train_sk/1/model_best.pth.tar', type=str, metavar='PATH')
parser.add_argument('--cpu', action="store_true", help='run on cpu')

args = parser.parse_args()
args.use_pose = ("photo" in args.train_mode)
# args.pretrained = not args.no_pretrained
args.result = os.path.join('..', 'result/v5/test_sk')
...
# 语义分割loss
road_criterion = nn.NLLLoss()
lane_criterion = nn.NLLLoss(weight=class_weight.cuda())
...
def acc(prediction, label, num_class=2):
    ...
    for i in range(bs):
        ...
        # precision = TP / TP + FP
        precision = precision+count[3]/(count[1]+count[3])

        # 当真实车道的数量大于100.即：FN+TP >100
    ...
    F1_score = 2*precision*acc_lane/(precision+acc_lane)
    F2_score = 5*precision*acc_lane/(4*precision+acc_lane)


    return acc_total, acc_lane,precision,F1_score,F2_score


# # 计算 recall 、 acc  、 precision 、F1
# def acc(prediction, label, num_class=2):
#     bs, c, h, w = prediction.size()
#     values, indices = prediction.max(1)
#
...
def iterate(mode, args, loader, model, optimizer, logger, best_acc, epoch):
    ...
    for i, batch_data in enumerate(loader):
        ...
        if mode == 'train':
            ...
            # 准确率
            #road_acc = acc(road_pred.data.cpu(), road_label.cpu())
            total_acc, lane_acc,precision_1,F1_score_1,F2_score_1 = acc(lane_pred.data.cpu(), lane_label.cpu())
            lane_acc_lst.append(lane_acc.item())
            total_acc_lst.append(total_acc.item())
            precision_list.append(precision_1.item())
            F1_score_list.append(F1_score_1.item())
            F2_score_list.append(F2_score_1.item())
            #print('total acc {}'.format(total_acc), 'lane acc {}'.format(lane_acc))
            #print('\n------------------------epoch '+str(epoch)+'------------------------------\n')
    ...
    if mode=='train':
        ...
        F1_score_mean = np.array(F1_score_list).mean()
        F2_score_mean = np.array(F2_score_list).mean()

        print('lane acc: {}'.format(lane_acc_mean), 'total acc: {}'.format(total_acc_mean),'precision: {}'.format(precision_mean) ,'F1 score: {}'.format(F1_score_mean),'F2 score: {}'.format(F2_score_mean))
    print('\n-------------------------epoch '+str(epoch)+'-----------------------------\n')
...
#    logger.conditional_summarize(mode, avg, is_best)
    ...
    if mode == 'train':
        ...
        return acc_avg, is_best, lane_acc_mean, total_acc_mean,precision_mean,F1_score_mean,F2_score_mean
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        start = time.clock()
        result, is_best, val_lane_acc, val_total_acc,val_precision,val_F1_score,val_F2_score = iterate("val", args, val_loader, model, None, logger, best_acc, epoch)  # evaluate on validation set
        ...
        val_F2_score_list.append(val_F2_score)
    ...
    #plot.plot('lane_val', args.epochs, lane_acc_list = val_lane_acc_list)
        ...
        with open('log/val_precision_v5_'+time.strftime('%Y-%m-%d-%H:%M:%S',time.localtime(time.time()))+'.txt', 'w+') as f:
            val_precision_list_w = [str(line) for line in val_precision_list]
            f.writelines('\n'.join(val_precision_list_w))
        with open('log/val_F1_score_v5_'+time.strftime('%Y-%m-%d-%H:%M:%S',time.localtime(time.time()))+'.txt', 'w+') as f:
            val_F1_score_list_w = [str(line) for line in val_F1_score_list]
            f.writelines('\n'.join(val_F1_score_list_w))
        with open('log/val_F1_score_v5_'+time.strftime('%Y-%m-%d-%H:%M:%S',time.localtime(time.time()))+'.txt', 'w+') as f:
            val_F2_score_list_w = [str(line) for line in val_F2_score_list]
            f.writelines('\n'.join(val_F2_score_list_w))
