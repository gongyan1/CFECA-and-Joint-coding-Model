...
#                            ps[-2:-1] + ['data'] + ps[-1:])
        ...
        if args.val == "select":
            ...
            glob_road_label = os.path.join(
                args.data_folder,
                #'data_depth_annotated/train/*_sync/proj_depth/groundtruth/image_0[2,3]/*.png'
                'small_data/test/road_label/*.png'
            )
            ...
            glob_lane_label = os.path.join(
                args.data_folder,
                #'data_depth_annotated/train/*_sync/proj_depth/groundtruth/image_0[2,3]/*.png'
                'small_data/test/lane_label/*.png'
            )
            
            # 4、
            glob_rgb = os.path.join(
                    args.data_folder,
                    'small_data/test/train_image_2_lane/*.png'
                )       
...
class KittiDepth(data.Dataset):
    ...
    def __getraw__(self, index):
        ...
        # 点云：原始强度
        point_cloud = depth_read(self.paths['pc'][index], self.args) if \
