...
args.result = os.path.join('..', 'results/v2')
...
def main():
    ...
    #plot.plot('lane_val', args.epochs, lane_acc_list = val_lane_acc_list)
        ...
        with open('log/train_lane_loss_v2_'+time.strftime('%Y-%m-%d-%H:%M:%S',time.localtime(time.time()))+'.txt', 'w+') as f:
            train_lane_loss_list_w = [str(line) for line in train_lane_loss_list]
            f.writelines('\n'.join(train_lane_loss_list_w))
        with open('log/train_lane_acc_v2_'+time.strftime('%Y-%m-%d-%H:%M:%S',time.localtime(time.time()))+'.txt', 'w+') as f:
            train_lane_acc_list_w = [str(line) for line in train_lane_acc_list]
            f.writelines('\n'.join(train_lane_acc_list_w))
        with open('log/train_total_acc_v2_'+time.strftime('%Y-%m-%d-%H:%M:%S',time.localtime(time.time()))+'.txt', 'w+') as f:
            train_total_acc_list_w = [str(line) for line in train_total_acc_list]
            f.writelines('\n'.join(train_total_acc_list_w))
        with open('log/val_lane_acc_v2_'+time.strftime('%Y-%m-%d-%H:%M:%S',time.localtime(time.time()))+'.txt', 'w+') as f:
            val_lane_acc_list_w = [str(line) for line in val_lane_acc_list]
            f.writelines('\n'.join(val_lane_acc_list_w))
        with open('log/val_total_acc_v2_'+time.strftime('%Y-%m-%d-%H:%M:%S',time.localtime(time.time()))+'.txt', 'w+') as f:
